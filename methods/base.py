...
from models import build_backbone, build_head, CLIP_MODELS, DINOv2_MODELS, BLIP_MODELS, BLIP2_MODELS
...
class BaseMethod(nn.Module):
    """Base class shared by every method in this repository.

    It owns the pieces that are independent of the adaptation algorithm:
    backbone construction, classifier head, checkpointing, and the default
    prediction / OOD-scoring interface. Subclasses override
    ``before_training`` / ``forward`` / ``predict``.
    """
    require_source = True
    require_target = False
    # set to True when ``forward`` needs the ground-truth target labels
    # (used only for logging pseudo-label quality, never for optimisation)
    require_target_labels = False
    # set to True for methods that take no gradient step: ``before_training``
    # does all the work and the run goes straight to evaluation
    training_free = False
    ...
    def __init__(self, cfg, tuning_config=None) -> None:
        ...
        self.backbone, self.vis_processors, self.txt_processors = build_backbone(cfg.backbone, tuning_config=tuning_config)
        self.cfg = cfg
        self.tuning_config = tuning_config
        if (self.tuning_config.ffn_adapt) & (self.cfg.fixed_backbone == False):
            for name, p in self.backbone.visual.named_parameters():
                if 'adapt' in name:
    ...
    def after_training(self):
        ...
        if self.cfg.save_checkpoint:
            torch.save(self.state_dict(), self.get_save_checkpoint_dir())

    def get_save_checkpoint_dir(self, fixed_backbone=None):
        return run_file(self.cfg, 'checkpoint.pth')

    def before_forward(self, batched_inputs = None):
        """
    ...
    def forward(self, batched_inputs):
        ...
        """
        normalize = Normalize(CLIP_PIXEL_MEAN, CLIP_PIXEL_STD)
        source_images = batched_inputs['source_images'].to(self.device)
        samples = {}
        if self.cfg.backbone in BLIP_MODELS+BLIP2_MODELS:
            tf = ToPILImage()
            source_images = [self.vis_processors["eval"](tf(image)).unsqueeze(0).to(self.device) for image in source_images]
            source_images = torch.cat(source_images)
            samples["image"] = source_images
            features = self.backbone.extract_features(samples, mode="image")
            features = features.image_embeds_proj
        else:
            source_images = normalize(source_images)
            if self.cfg.backbone in CLIP_MODELS:
                features = self.backbone.visual(source_images)
            else:
    ...
    def predict(self, batched_inputs):
        ...
        with torch.no_grad():
            ...
            if self.cfg.backbone in BLIP_MODELS+BLIP2_MODELS:
                images = [self.vis_processors["eval"](tf(image)).unsqueeze(0).to(self.device) for image in images]
                samples["image"] = torch.cat(images)
                features = self.backbone.extract_features(samples, mode='image')
                features = features.image_embeds_proj
            else:
                images = normalize(images)
                if self.cfg.backbone in CLIP_MODELS:
                    features = self.backbone.visual(images)
                else:
